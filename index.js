 ...
 * it will return `This is a Lincoln Navigator`.
*/
function getCarInfoById(inventory, idParam) {
  /* code here */
...
function sortCarInventory(inventory) {
  return inventory.sort(function(a,b){
    ...
    else if (b.car_model > a.car_model){
      comparison = -1;
    }
    return comparison;
  })
...
function getGermanCars(inventory) {
  const germanBrands = ['Audi', 'Mercedes-Benz', 'Volkswagen', 'BMW'];
  let germanCars = [];
  for (car of inventory){
    if (germanBrands.indexOf(car.car_make) !== -1){
      germanCars.push(car);
    }
  }
  return germanCars;
